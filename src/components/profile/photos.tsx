/* eslint-disable no-nested-ternary */
// @ts-expect-error ts-migrate(7016) FIXME: Could not find a declaration file for module 'prop... Remove this comment to see the full error message
import PropTypes from 'prop-types';
import Skeleton from 'react-loading-skeleton';

export default function Photos({
  photos
}: any) {
  return (
    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'div'.
    <div className="h-16 border-t border-gray-primary mt-12 pt-4">
      // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'div'.
      <div className="grid grid-cols-3 gap-8 mt-4 mb-12">
        {!photos ? (
          <>
            // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'count'.
            <Skeleton count={12} width={320} height={400} />
          </>
        ) : photos.length > 0 ? (
          // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'div'.
          photos.map((photo: any) => <div key={photo.docId} className="relative group">
            // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'img'.
            <img src={photo.imageSrc} alt={photo.caption} />
            // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'div'.
            <div className="absolute bottom-0 left-0 bg-gray-200 z-10 w-full justify-evenly items-center h-full bg-black-faded group-hover:flex hidden">
              // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'p'.
              <p className="flex items-center text-white font-bold">
                // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'svg'.
                <svg
                  // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'xmlns'.
                  xmlns="http://www.w3.org/2000/svg"
                  // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'className'.
                  className="w-8 mr-4"
                  // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'viewBox'.
                  viewBox="0 0 20 20"
                  // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'fill'.
                  fill="currentColor"
                >
                  // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'path'.
                  <path
                    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'fillRule'.
                    fillRule="evenodd"
                    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'd'.
                    d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z"
                    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'clipRule'.
                    clipRule="evenodd"
                  />
                </svg>
                // @ts-expect-error ts-migrate(2365) FIXME: Operator '<' cannot be applied to types '{ photo: ... Remove this comment to see the full error message
                {photo.likes.length}
              </p>
              // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'p'.
              <p className="flex items-center text-white font-bold">
                // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'svg'.
                <svg
                  // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'xmlns'.
                  xmlns="http://www.w3.org/2000/svg"
                  // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'className'.
                  className="w-8 mr-4"
                  // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'viewBox'.
                  viewBox="0 0 20 20"
                  // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'fill'.
                  fill="currentColor"
                >
                  // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'path'.
                  <path
                    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'fillRule'.
                    fillRule="evenodd"
                    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'd'.
                    d="M18 10c0 3.866-3.582 7-8 7a8.841 8.841 0 01-4.083-.98L2 17l1.338-3.123C2.493 12.767 2 11.434 2 10c0-3.866 3.582-7 8-7s8 3.134 8 7zM7 9H5v2h2V9zm8 0h-2v2h2V9zM9 9h2v2H9V9z"
                    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'clipRule'.
                    clipRule="evenodd"
                  />
                </svg>
                // @ts-expect-error ts-migrate(2365) FIXME: Operator '<' cannot be applied to types '{ photo: ... Remove this comment to see the full error message
                {photo.comments.length}
              </p>
            </div>
          </div>)
        ) : null}
      </div>
      // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'photos'.
      {!(photos?.length ?? 0) && (
        // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'p'.
        <p className="text-center text-2xl">No Posts Yet</p>
      )}
    </div>
  );
}

Photos.propTypes = {
  photos: PropTypes.array.isRequired,
};
